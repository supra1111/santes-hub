...
local function SantesHub()
    ...
    local function buildMainGui()
        ...
        InfoLabel.Text      = "⚡  Glow + İsim + Takma İsim  •  harita sınırında gizlenir"
        ...
        local function makeTagTemplate()
            ...
            nameTag.Size               = UDim2.new(1, -8, 0, 24)
            ...
            dispTag.Position           = UDim2.new(0, 4, 0, 26)
            dispTag.Font               = Enum.Font.Gotham
            dispTag.TextSize           = 10
            dispTag.TextColor3         = Color3.fromRGB(255, 150, 150)
            dispTag.TextStrokeColor3   = Color3.fromRGB(0, 0, 0)
            dispTag.TextStrokeTransparency = 0.3
            dispTag.Text               = ""
        ...
        local function watchPlayerForESP(plr)
            if plr == LocalPlayer then return end
            
            local addConn = plr.CharacterAdded:Connect(function()
                task.wait(1)
                if toggled then
                    pcall(function() createESPForPlayer(plr) end)
                end
            ...
            espConnections[plr] = { addConn, remConn }
        end

        local function unwatchPlayerForESP(plr)
            if espConnections[plr] then
                for _, c in ipairs(espConnections[plr]) do
